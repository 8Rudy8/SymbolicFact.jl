...
struct Snode
	...
	Snode(p,x,y,z,t,e) = new(p,x,y,z,t,e) #with all params
	Snode() = new(0,0,0,Set(),Set(),1)#empty node
end
"""
...
function symbolicfact(A,supernodes = true)
	...
	if issymmetric(A)
		...
		for i = 1:n
			...
			result[i] = Snode(elimtree[i],rowcount[i],-1,idxr[i],Set(),np[i])
		end
	else
		...
		for i = 1:n
			result[i] = Snode(elimtree[i],rowcount[i],colcount[i], Set(),Set(),np[i])
		end
	end
	return result
end
